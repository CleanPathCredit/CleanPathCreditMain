 ...
 *
 * GET  /api/admin/messages?profileId=xxx  → messages thread for a client (admin only)
 * POST /api/admin/messages                → send a message to a client (admin only)
 *
 * Body for POST: { profileId: string; body: string }
 */
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === "GET") {
    ...
    if (!profileId) {
      return new Response(JSON.stringify({ error: "Missing profileId" }), { status: 400, headers: JSON_H });
    }
  ...
  if (req.method === "POST") {
    const body = (await req.json()) as { profileId: string; body: string };
    if (!body.profileId || !body.body?.trim()) {
      return new Response(JSON.stringify({ error: "Missing profileId or body" }), { status: 400, headers: JSON_H });
    }
    const { error } = await supabase
      .from("messages")
      .insert({ profile_id: body.profileId, sender: "admin", body: body.body.trim() });
